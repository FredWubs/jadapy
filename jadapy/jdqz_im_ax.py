...
def jdqz(A, B, num=5, max_cnt=5, target=Target.SmallestMagnitude, tol=1e-8, lock_tol=None, prec=None,
    ...
    num_mx_cnt=num*max_cnt    
    # Generalized eigenvalues
    aconv = numpy.zeros(num_mx_cnt + extra, ctype)
    bconv = numpy.zeros(num_mx_cnt + extra, dtype)

    # Generalized Schur matrices
    RA = numpy.zeros((num_mx_cnt + extra, num_mx_cnt + extra), dtype)
    RB = numpy.zeros((num_mx_cnt + extra, num_mx_cnt + extra), dtype)

    # Generalized Schur vectors
    Q = interface.vector(num_mx_cnt + extra)
    Z = interface.vector(num_mx_cnt + extra)
    # Preconditioned Z
    Y = interface.vector(num_mx_cnt + extra)
    QZ = numpy.zeros((num_mx_cnt + extra, num_mx_cnt + extra), dtype)
      ...
      while k < cnt*num and it <= maxit:
        ...
        while found:
            ...
            if rnorm <= tol and m > nev:
                ...
                for i in range(nev):
                    print("Found  %d-th eigenvalue: %e+%ei" %(k+1,(evs[0, i] / evs[1, i]).real,(evs[0, i] / evs[1, i]).imag ), flush=True)
                    # print(evs[0, i], evs[1, i])
                    aconv[k] = evs[0, i]
                    # FW removed .real from next line. I found that evs[1,i] is purely imaginary 
                    bconv[k] = evs[1, i]
                    k += 1
        ...
        if m >= min(subspace_dimensions[1], n - k) and k < num_mx_cnt:
        ...
        it += 1
      print('finished inner loop', cnt)
      #maxval=max(abs(complex(aconv[(cnt-1)*num:cnt*num],aconv[(cnt-1)*num:cnt*num])))
      #printaconv[(cnt-1)*num:cnt*num])+numpy.square(aconv[(cnt-1)*num:cnt*num])))
      maxval=max(abs(aconv[(cnt-1)*num:cnt*num]/bconv[(cnt-1)*num:cnt*num]-target))
      target=target+ complex(0.0,maxval)
      print('local sp. radius and new target', maxval, target)
      cnt=cnt+1
